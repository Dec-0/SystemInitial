...
if ${Flag4R} && [ ! -s ${Dir4Work}/R-3.6.3/bin/R ] ; then
	...
	export CFLAGS="$CFLAGS -I${Dir4Work}/bzip2-1.0.6/include"
	export LDFLAGS="$LDFLAGS -L${Dir4Work}/bzip2-1.0.6/lib"
	cd R-3.6.3 && ./configure --prefix=${Dir4Work}/R-3.6.3
	make && make install
...
############ bedtools
if ${Flag4Bedtools} && [ ! -s ${Dir4Work}/bedtools-2.26/bin/bedtools ] ; then
	[[ ! -d ${Dir4Work}/SourcePackages ]] && mkdir -p ${Dir4Work}/SourcePackages
	cd ${Dir4Work}/SourcePackages
